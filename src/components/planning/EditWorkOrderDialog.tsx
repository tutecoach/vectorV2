...
import { useUpdateWorkOrder, useTechnicians } from "@/hooks/usePlanning";
...
// Extendemos los campos de la tabla base para incluir la info relacional
...
export default function EditWorkOrderDialog({ open, onClose, workOrder }: EditWorkOrderDialogProps) {
    ...
    // Resetear formulario cuando cambia la OT seleccionada
    useEffect(() => {
        if (workOrder && open) {
            form.reset({
                ...
                scheduled_date: workOrder.scheduled_date ? new Date(workOrder.scheduled_date).toISOString().split('T')[0] : "",
                flexible_schedule: workOrder.flexible_schedule || false,
                status: (workOrder.status as any) || "pendiente",
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        try {
            ...
            toast({
                ...
                description: error?.message || "Ha ocurrido un error inesperado al actualizar la orden.",
            });
        }
    };
    ...
    return (
        <Dialog open={open} onOpenChange={(val) => !val && onClose()}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle className="font-heading">Detalle / Editar Orden {workOrder.id.split("-").pop() || workOrder.id}</DialogTitle>
                </DialogHeader>
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <Select onValueChange={field.onChange} value={field.value}>
                                        ...
                                        <SelectContent>
                                            {clients?.filter(c => c.status === "activo" || c.id === field.value).map((c) => (
                                ...
                                <FormItem>
                                    ...
                                    {!isAddingSite ? (
                                        ...
                                        <div className="flex gap-2">
                                            <Input
                                                autoFocus
                                                placeholder="Nombre de la nueva sede/zona"
                                                value={newSiteName}
                        ...
                        <div className="flex justify-end gap-2 pt-4">
                            ...
                            <Button type="submit" disabled={updateWorkOrder.isPending || (!form.formState.isDirty)}>
                                {updateWorkOrder.isPending ? "Guardando..." : "Guardar Cambios"}
                            </Button>
                        </div>
                    </form>
                </Form>
